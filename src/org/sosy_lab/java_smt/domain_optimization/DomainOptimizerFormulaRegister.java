        ...
        new FormulaVisitor<>() {
          ...
          public Formula visitFreeVariable(Formula f, String name) {
            ...
            if (dec == FunctionDeclarationKind.LTE) {
              IntegerFormula upperBound = imgr.makeNumber(domain.getLowerBound());
              return upperBound;
            } else if (dec == FunctionDeclarationKind.LT) {
              IntegerFormula upperBound = imgr.makeNumber(domain.getLowerBound() + 1);
              return upperBound;
            } else if (dec == FunctionDeclarationKind.GTE) {
              IntegerFormula lowerBound = imgr.makeNumber(domain.getUpperBound());
              return lowerBound;
            } else if (dec == FunctionDeclarationKind.GT) {
              IntegerFormula lowerBound = imgr.makeNumber(domain.getUpperBound() - 1);
              return lowerBound;
            }
            return null;
          }
